...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        pe = pe.unsqueeze(0)
        # fixing positional encodings as it doesn't need to be learned
...
class LayerNormalization(nn.Module):
    ...
    def __init__(self, eps: float = 10 ** -6) -> None:
        super().__init__()
        self.eps = eps
        # defining the learnable parameters using nn.Parameter
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        # d_v = d_k and h * d_v = d_model (d_k, d_v = d_model // h)
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512,
    ...
    # Create the positional embedding layers (Although we can use only one pos layer as it's same, we're just doing
    # it for understanding purpose)
    ...
    transformer = Transformer(encoder, decoder, src_embed, tgt_embed, src_pos, tgt_post, projection_layer)
